...
- **Install CLI tools.** Python packages providing executables (bon, passe, accomplis) need `uv tool install` separately — the suite's session hooks do this install-if-missing. The plugin handles skills/hooks/MCP but not PATH binaries.
