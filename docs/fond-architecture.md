...
**Key insight: things in .bon/ (git-tracked) are supple — they sync across machines naturally. Things in ~/.claude/ are brittle — per-machine, per-path, at Anthropic's mercy.**
...
Reads handoff → orients from Now zone → synthesizes Learned section into understanding.md (this is onboarding, not busywork — the act of integrating forces the new Claude to build a working mental model). Handoff stays on disk after processing.
...
### Feeding Anthropic's System

We don't write to MEMORY.md directly. The handoff content is in the session transcript (written via Write tool). When autoDream processes the transcript, it finds a pre-structured, pre-reflected summary — better signal than raw tool calls. The richer our handoff, the better autoDream's consolidation.
