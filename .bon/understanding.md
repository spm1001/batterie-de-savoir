...
Five published plugins: **batterie, bon, trousse, mise, todoist-gtd** (public marketplace), plus **mise-home** in the private `batterie-home` marketplace. **passe was delisted 2026-07-07** (bds-wobari via bds-mumise: browser infra, not a knowledge plugin — CLI installs standalone; the orphaned `passe@batterie` install lingers on Sameer's machines pending the shard-re-home migration, bds-tujoro). garde-manger was decommissioned 2026-06-03. guéridon is infrastructure (a token-usage gauge), never a published plugin. Post-japoca (2026-07-06) version semantics: every published plugin carries the suite number; a CLI's `--version` reads *the suite release that last changed that CLI* (publish.py lazy-stamps only the repo being published); session hooks are install-if-missing only; `/batterie:update` detects CLI drift by **git commit** (direct_url.json vs `ls-remote`), with provenance-sticky install sources (a `~/repos` clone appearing no longer flips installs onto the working tree).
